...
class CredentialsCompilerPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container)
    {
        ...
        $establishedCredentials[] = new Definition(
            'SymfonyFolder\DotpayBundle\Credentials\ParametersCredentials', [
                new Reference('service_container')
            ]
        );

        $establishedCredentials[] = new Definition(
            'SymfonyFolder\DotpayBundle\Credentials\ExtensionCredentials', [
                new Reference('service_container')
            ]
        );

        $container->setDefinition('dotpay_credentials', new Definition(
            'SymfonyFolder\DotpayBundle\Credentials\CachedCredentials', [
                new Definition(
                    'SymfonyFolder\DotpayBundle\Credentials\EstablishedCredentialsChain',
                    $establishedCredentials
                )
            ]
        ));
